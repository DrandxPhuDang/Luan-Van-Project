...
class data_preprocess:
    ...
    def __init__(self, path_train_test):
        ...
        def preprocessing_data(X_data):
            ...
            X_data = pd.DataFrame(X_data, columns=full_waves)
            return X_data

        self.X_train = preprocessing_data(self.X_train)
        self.X_test = preprocessing_data(self.X_test)
...
class data_predict_regression:
    ...
    def __init__(self, model_regression, path_file_data, preprocess):
        ...
        if model_regression == 'C':
            name_model_pls = 'Model'
        ...
        if model_regression == 'SR':
            name_model_knn = 'Staking'

            base_models = [
                ('rf', PLSRegression(n_components=3)),
                ('p', RandomForestRegressor(n_estimators=59, random_state=42)),
                # ('g', GaussianProcessRegressor(kernel=RBF(length_scale=1.0), alpha=0.1))
            ]

            # model_knn = PLSRegression(n_components=2)
            # model_knn = RandomForestRegressor(n_estimators=59, random_state=42, max_depth=5)
...
if __name__ == "__main__":
    ...
    # --------------------------------------REGRESSION DATA-------------------------------------------------------------
    data_predict_regression(model_regression='K',
